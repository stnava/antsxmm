...
def test_process_session_t1_filter_and_dwi_persistence(mock_multi_run_data, tmp_path):
  ...
  with patch("antspymm.generate_mm_dataframe") as mock_gen, \
    ...
    # We use "placeholder" to verify that antsxmm DOES NOT overwrite this value.
    ...
    # The logic used to write back "r001" (parsed from filename).
    # We REMOVED that logic, so now we expect the value from mock_gen ("placeholder")
    # to remain untouched.
